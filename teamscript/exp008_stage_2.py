...
class CFG:
    ...
    # Data #
    ######################
    train_datadir = Path("/kaggle/input/birdclef-2021/train_short_audio")
    train_csv = "/kaggle/input/birdclef-2021/train_soundscape_labels.csv"
    train_soundscape = "/kaggle/input/birdclef-2021/train_soundscape_labels.csv"
    birdcall_datadir = Path(
        "/kaggle/input/birdclef-2021/train_short_audio")
    birdcall_csv = "/kaggle/input/birdclef-2021/train_metadata.csv"
...
class WaveformStageTwoDataset(torchdata.Dataset):
    def __init__(self,
                 df: pd.DataFrame,
                 cached_clips: dict,
                 img_size=224,
                 ...
                 validation=False):
        self.df = df
        self.cached_clips = cached_clips
        self.waveform_transforms=waveform_transforms
    ...
    def __getitem__(self, idx: int):
        ...
        y = self.cached_clips[audio_id][start_index:end_index].astype(np.float32)
        ...
        labels = np.zeros(len(CFG.target_columns), dtype=float)
        for l in ebird_code.split(" "):
            if l != 'nocall':
                labels[CFG.target_columns.index(l)] = 1.0
...
if __name__ == "__main__":
    ...
    # cache train files
    import glob
    audios = glob.glob(f"/kaggle/input/birdclef-2021/train_soundscapes/*")
    cached_clips = {}
    for i, x in enumerate(audios):
        row_id = int(os.path.basename(x).split("_")[0])
        cached_clips[row_id] = x
        clip, _ = sf.read(x)
        cached_clips[row_id] = clip
    ...
    nocall_idx = train[(train.birds == 'nocall')].index.values
    nocall_idx = nocall_idx[:int(len(nocall_idx) * 0.90)]
    print(nocall_idx)
    print(len(train))
    train = train.drop(nocall_idx).reset_index(drop=True)
    print(len(train))
    print("dropped 90% nocall")
    ...
    if CFG.train:
        for i, (trn_idx, val_idx) in enumerate(splitter.split(train, y=train["birds"])):
            print(i)
            ...
            ckpt = torch.load("../out/exp008_1/fold0/checkpoints/best.pth", 
                              map_location='cpu')['model_state_dict']
            model.load_state_dict(ckpt)
            ## 1st model training with last.pth
