...
class WaveformDataset(torchdata.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if not self.validation:
            for _ in range(self.n_mix):
                if np.random.rand() < 0.5:
                    ...
                    ebird_code = sample["primary_label"]
                    secondary_labels = eval(sample["secondary_labels"])
                    y_, sr = sf.read(self.datadir / ebird_code / wav_name)
                    ...
                    if len_y < effective_length:
                        ...
                        new_y[start:start + len_y] = y_
                        y_ = new_y.astype(np.float32)
                    elif len_y > effective_length:
                        start = np.random.randint(len_y - effective_length)
                        y_ = y_[start:start +
                                effective_length].astype(np.float32)
                    else:
                        y_ = y_.astype(np.float32)
                    y_ = np.nan_to_num(y_)
                    if np.isnan(y_).any():
                        y_ = np.zeros(len(y_), dtype=np.float32)
        ...
        y_background = np.nan_to_num(y_background)
        if np.isnan(y_background).any():
            y_background = np.zeros(len(y_background))

        y_normalized = y / np.abs(y).max()
        y_background_normalized = y_background / np.abs(y_background).max()
