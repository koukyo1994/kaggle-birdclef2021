...
class CFG:
    ...
    soft_label_dir = Path("../out/pl000_exp32_5fold/soft_labels")
    ...
    hop_length = 768
    sample_rate = 32000
    primary_label_threshold = 0.3
    secondary_labels_threshold = 0.3
    other_labels_threshold = 0.9
...
class TorchAudioWaveformDataset(torchdata.Dataset):
    def __init__(self,
        ...
        self.soft_label_dir = CFG.soft_label_dir
    ...
    @staticmethod
    def index2seconds(index: int):
        ratio = 20 / 27
        return index * ratio

    @staticmethod
    def seconds2index(seconds: float):
        ratio = 27 / 20
        return int(seconds * ratio)

    def index2frame(self, index: int):
        seconds = self.index2seconds(index)
        SR = 32000
        return int(SR * seconds)

    def frame2index(self, frame: int):
        SR = 32000
        seconds = frame / SR
        return self.seconds2index(seconds)
    ...
    def __getitem__(self, idx: int):
        ...
        soft_label = np.load(self.soft_label_dir /
                             (wav_name + ".npz"))["arr_0"]

        labels = torch.zeros(len(CFG.target_columns), dtype=torch.float)

        SR = 32000
        label_indices = list(range(len(CFG.target_columns)))
        used_label = [CFG.target_columns.index(ebird_code)]
        for secondary_label in secondary_labels:
            if secondary_label in CFG.target_columns:
                used_label.append(CFG.target_columns.index(secondary_label))
        other_labels = sorted(set(label_indices) - set(used_label))

        y, start = self.__read_audio(
            self.datadir / ebird_code / wav_name, duration)
        labels[CFG.target_columns.index(ebird_code)] = 1.0

        soft_label_start_idx = self.frame2index(start)
        soft_label_end_idx = self.frame2index(start + SR * self.period)

        for secondary_label in secondary_labels:
            if secondary_label in CFG.target_columns:
                secondary_label_pred = soft_label[
                    soft_label_start_idx:soft_label_end_idx,
                    CFG.target_columns.index(secondary_label)]
                if (secondary_label_pred > CFG.secondary_labels_threshold).any():
                    labels[CFG.target_columns.index(secondary_label)] = 1.0

        if (soft_label[
            soft_label_start_idx:soft_label_end_idx,
                other_labels].max(axis=0) > CFG.other_labels_threshold).any():
            positions = np.argwhere(
                soft_label[
                    soft_label_start_idx:soft_label_end_idx,
                    other_labels].max(axis=0) > CFG.other_labels_threshold
            ).reshape(-1)
            for pos in positions:
                labels[other_labels[pos]] = 1.0
        ...
        if not self.validation:
            for _ in range(self.n_mix):
                if np.random.rand() < 0.5:
                    ...
                    soft_label = np.load(
                        self.soft_label_dir / (wav_name + ".npz"))["arr_0"]
                    ...
                    labels[CFG.target_columns.index(ebird_code)] = 1.0
                    soft_label_start_idx = self.frame2index(start)
                    soft_label_end_idx = self.frame2index(
                        start + SR * self.period)
        ...
        if np.random.rand() < 0.5 and not self.validation:
            n_steps = np.random.choice(np.arange(-5.0, 5.0, 0.1))
...
if __name__ == "__main__":
    ...
    train = train.query("allzero == False").reset_index(drop=True)
