...
class TorchAudioWaveformDataset(torchdata.Dataset):
    ...
    def __read_audio(self, path: Path, duration: float):
        ...
        if len_y < effective_length:
            ...
            if not self.validation:
                start = torch.randint(
                    0, effective_length - len_y, size=(1,)).item()
            else:
                start = 0
            ...
            if not self.validation:
                start = torch.randint(
                    0, len_y - effective_length, size=(1,)).item()
            else:
                start = 0
            y, _ = torchaudio.load(
    ...
    def __getitem__(self, idx: int):
        ...
        if not self.validation:
            for _ in range(self.n_mix):
                if torch.rand((1,)).item() < 0.5:
                    idx_ = torch.randint(
                        0, len(self.filenames), size=(1,)).item()
                    if idx_ == idx:
                        continue
        ...
        if torch.rand((1,)).item() < 0.5 and not self.validation:
            options = np.arange(-5.0, 5.0, 0.1)
            idx_op = torch.randint(0, len(options), (1,)).item()
            n_steps = options[idx_op]
            y = self.__resolve_nan(y)
            y = torch.from_numpy(
