...
class CFG:
    ...
    train_soundscape_datadir = Path("../input/birdclef2020/test_audio")
    train_csv = "../input/birdclef-2021/train_metadata.csv"
    train_soundscape = "../input/birdclef2020/test.csv"
...
class WaveformSoundscapeDataset(torchdata.Dataset):
    def __init__(self,
                 df: pd.DataFrame,
                 datadir: Path,
                 waveform_transforms=None):
        ...
        self.waveform_transforms = waveform_transforms
        audio_id2path = {}
        all_audio_ids = self.df["row_id"].map(
            lambda x: "_".join(x.split("_")[:2])).unique()
        for audio_id in all_audio_ids:
            audio_path = list(self.datadir.glob(f"{audio_id}*"))[0]
            audio_id2path[audio_id] = audio_path

        self.audio_id2path = audio_id2path
    ...
    def __getitem__(self, idx: int):
        sample = self.df.loc[idx, :]
        audio_id = "_".join(sample.row_id.split("_")[:2])
        audio_path = self.audio_id2path[audio_id]

        end_second = sample["seconds"]
        start_second = end_second - 5
        sr = 32000
        start_index = start_second * sr
        frames = sr * 5

        y, sr = sf.read(audio_path, frames=frames,
                        start=start_index, dtype=np.float32)
        ...
        y = np.nan_to_num(y)
        labels = np.zeros(len(CFG.target_columns), dtype=float)
        if sample.birds == "nocall":
            pass
        else:
            birds = sample.birds.split(" ")
            for bird in birds:
                if bird in CFG.target_columns:
                    labels[CFG.target_columns.index(bird)] = 1.0
...
class SampleF1Callback(Callback):
    ...
    def on_batch_end(self, state: IRunner):
        ...
        if state.is_valid_loader:
            y_pred_raw = np.zeros(
                (len(clipwise_output), len(CFG.target_columns) + 1))
            y_targ_raw = np.zeros(
                (len(clipwise_output), len(CFG.target_columns) + 1)
            )
        ...
        if state.is_valid_loader:
            y_pred_raw[:, :len(CFG.target_columns)] = y_pred
            nocall = (y_pred.mean(axis=1) == 0.0).astype(np.float32)
            y_pred_raw[:, len(CFG.target_columns)] = nocall
            y_pred = y_pred_raw

            y_targ_raw[:, :len(CFG.target_columns)] = targ
            nocall = (targ.mean(axis=1) == 0.0).astype(np.float32)
            y_targ_raw[:, len(CFG.target_columns)] = nocall
            targ = y_targ_raw
    ...
    def on_loader_end(self, state: IRunner):
        ...
        if state.is_valid_loader:
            y_pred_raw = np.zeros((len(y_pred), len(CFG.target_columns) + 1))
            y_pred_raw[:, :len(CFG.target_columns)] = y_pred
            nocall = (y_pred.mean(axis=1) == 0.0).astype(np.float32)
            y_pred_raw[:, len(CFG.target_columns)] = nocall
            y_pred = y_pred_raw
            ...
            y_targ_raw = np.zeros(
                (len(y_true), len(CFG.target_columns) + 1)
            )
            y_targ_raw[:, :len(CFG.target_columns)] = y_true
            nocall = (y_true.mean(axis=1) == 0.0).astype(np.float32)
            y_targ_raw[:, len(CFG.target_columns)] = nocall
            y_true = y_targ_raw
...
if __name__ == "__main__":
    ...
    train_soundscape_labels = pd.read_csv(CFG.train_soundscape)
    ...
    if CFG.train:
        ...
        loaders["valid"] = torchdata.DataLoader(
            WaveformSoundscapeDataset(
                train_soundscape_labels,
                CFG.train_soundscape_datadir,
