...
TARGET_SITE = "COR"
site_species_map = pd.read_csv("../input/birdclef-2021/site_species_map.csv")
target_species = sorted(site_species_map.query(
    f"{TARGET_SITE} == 1")["species"].tolist())
...
if __name__ == "__main__":
    ...
    train = train[train["primary_label"].isin(
        target_species)].reset_index(drop=True)
    train_background_paths = list(CFG.train_background.glob("*.wav"))
    train_background_paths = [
        path for path in train_background_paths if TARGET_SITE in path.name]
