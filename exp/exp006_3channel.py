...
class MultiChannelDataset(torchdata.Dataset):
    def __init__(self,
                 df: pd.DataFrame,
                 datadir: Path,
                 img_size=224,
    ...
    if CFG.train:
        for i, (trn_idx, val_idx) in enumerate(splitter.split(train, y=train["primary_label"])):
            ...
            loaders = {
                phase: torchdata.DataLoader(
                    MultiChannelDataset(
                        ...
                        spectrogram_transforms=None,
                        melspectrogram_parameters=CFG.melspectrogram_parameters,
                        pcen_parameters=CFG.pcen_parameters,
                        period=20,
            ...
            model = EfficientNetSED(
                ...
                pretrained=False,
                num_classes=264)
            ckpt = torch.load("../bin/EfficientNetSED/best.pth")
            model.load_state_dict(ckpt["model_state_dict"])
            in_features = model.fc1.out_features
            model.att_block = AttBlockV2(
